...
export async function POST(request: NextRequest) {
  ...
  const focus = typeof payload.focus === "string" ? payload.focus.trim() : "bugs, security, best practices";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!code) {
    return NextResponse.json({ error: "Request must include code to review." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's code review expert. Analyze the provided code in Markdown with sections: Summary, Issues Found (categorized by severity: Critical, High, Medium, Low), Security Concerns, Performance Considerations, Best Practices Violations, and Recommendations. Be thorough but constructive.",
    },
    {
      role: "user",
      content: `Review this code focusing on: ${focus}\n\n\`\`\`\n${code}\n\`\`\``,
    },
  ];
