...
export async function POST(request: NextRequest) {
  ...
  const examples = typeof payload.examples === "string" ? payload.examples.trim() : "";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!description) {
    return NextResponse.json({ error: "Request must describe the regex pattern needed." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's regex builder. Generate regular expressions in Markdown with sections: Regex Pattern (the expression in a code block), Pattern Breakdown (explain each component), Flags (recommended flags and why), Test Cases (examples that match and don't match), Common Pitfalls (edge cases to watch), and Alternative Patterns (if applicable). Make patterns readable and maintainable.",
    },
    {
      role: "user",
      content: `Create a regex pattern for: ${description}\n\nExample strings to match:\n${examples || "(no examples provided)"}`,
    },
  ];
