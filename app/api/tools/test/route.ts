...
export async function POST(request: NextRequest) {
  ...
  const framework = typeof payload.framework === "string" ? payload.framework.trim() : "Jest";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!code) {
    return NextResponse.json({ error: "Request must include code to generate tests for." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's test generation specialist. Generate comprehensive unit tests in Markdown with sections: Test Suite (complete test code in a fenced code block), Test Coverage (what scenarios are covered), Edge Cases (boundary conditions tested), Setup/Teardown (if needed), and Assertions Explained. Follow testing best practices and framework conventions.",
    },
    {
      role: "user",
