...
export async function POST(request: NextRequest) {
  ...
  const schemaText = typeof payload.schema === "string" ? payload.schema.trim() : "";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!requestText) {
    return NextResponse.json({ error: "Request must describe the SQL to generate." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's SQL tutor. Given the user request and optional schema, return Markdown with two sections: SQL Query (a fenced sql code block) and Explanation (bullet list). Validate table and column names against the provided schema when present and note assumptions explicitly.",
    },
    {
      role: "user",
      content: `Request: ${requestText}\n\nSchema (may be empty):\n${schemaText || "(not provided)"}`,
    },
  ];
