...
export async function POST(request: NextRequest) {
  ...
  const style = typeof payload.style === "string" ? payload.style.trim() : "JSDoc";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!code) {
    return NextResponse.json({ error: "Request must include code to document." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's documentation generator. Create comprehensive documentation in Markdown with sections: Documentation (formatted comments in specified style), Summary (high-level overview), Parameters/Arguments (detailed descriptions), Return Values, Exceptions/Errors, and Usage Examples. Follow the specified documentation style conventions.",
    },
    {
      role: "user",
