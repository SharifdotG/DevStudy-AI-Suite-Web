...
export async function POST(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.json({ error: "Request must include a code snippet to explain." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's code mentor. Explain the provided code clearly in Markdown with sections: Overview, Step-by-step Walkthrough, Potential Pitfalls. Include language-appropriate syntax highlighting and suggest improvements when relevant.",
    },
    {
      role: "user",
