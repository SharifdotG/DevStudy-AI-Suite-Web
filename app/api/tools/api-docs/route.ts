...
export async function POST(request: NextRequest) {
  ...
  const format = typeof payload.format === "string" ? payload.format.trim() : "OpenAPI";
  const model = typeof payload.model === "string" && payload.model.trim().length ? payload.model : DEFAULT_MODEL;

  if (!code) {
    return NextResponse.json({ error: "Request must include API code to document." }, { status: 400 });
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are DevStudy's API documentation specialist. Generate comprehensive API documentation in Markdown with sections: API Overview, Endpoints (with HTTP methods, paths, descriptions), Request Parameters (path, query, body), Response Formats (status codes and payloads), Authentication, Rate Limiting, and Example Requests/Responses. Format according to the specified documentation standard.",
    },
    {
      role: "user",
      content: `Generate ${format} API documentation for these endpoints:\n\n\`\`\`\n${code}\n\`\`\``,
    },
  ];
